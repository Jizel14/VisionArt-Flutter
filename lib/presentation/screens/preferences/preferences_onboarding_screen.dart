...
class _PreferencesOnboardingScreenState
  ...
  bool _saving = false;
  String _savingMessage = 'Enregistrement de vos préférences...';
  ...
  Future<void> _complete() async {
    setState(() {
      _saving = true;
      _savingMessage = 'Enregistrement de vos préférences...';
    });
    ...
    try {
      ...
      debugPrint('Preferences sync skipped: $e');
    }

    if (!mounted) return;
    setState(() {
      _savingMessage = 'Generation de vos 3 musiques...';
    });

    await _generateInitialPlaylist(p);
  ...
  void _finishOnboarding() {
    if (widget.initialPreferences == null && Navigator.of(context).canPop()) {
      Navigator.of(context).pop();
    }
    widget.onComplete();
  }

  Future<void> _generateInitialPlaylist(UserPreferences preferences) async {
    try {
      final urls = await AudioService().generatePlaylistFromSavedPreferences();

      if (urls.isNotEmpty) {
        final playlist = SonicPlaylist(
          id: DateTime.now().millisecondsSinceEpoch.toString(),
          name:
              'Mon univers ${preferences.styles.isNotEmpty ? preferences.styles.first : 'IA'}',
          urls: urls,
      ...
      debugPrint('Playlist generation skipped: $e');
    }
  }
