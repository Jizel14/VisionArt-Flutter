...
class AudioService {
  ...
  static final _audioGenerationOptions = Options(
  ...
  Future<List<String>> generatePlaylist({
    ...
    try {
      final response = await ApiClient.instance.post(
        ...
        options: _audioGenerationOptions,
      );
  ...
  /// Generate a 3-track playlist from preferences already saved on the backend.
  /// Returns absolute URLs.
  Future<List<String>> generatePlaylistFromSavedPreferences() async {
    try {
      final response = await ApiClient.instance.post(
        '/user-preferences/generate-playlist',
        options: _audioGenerationOptions,
      );
      final data = response.data;
      if (data is Map && data['tracks'] is List) {
